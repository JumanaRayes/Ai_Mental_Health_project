...
class MistralChatbot:
    ...
    def load_model(self):
        ...
        if self.device == "cuda":
            ...
            print("WARNING: CUDA not detected. Loading full model on CPU. This will consume high RAM and be slow.")
            self.model = AutoModelForCausalLM.from_pretrained(
                self.model_id,
                device_map="cpu",
    ...
    def generate_response(self, user_message: str, max_new_tokens: int = 250, temperature: float = 0.7) -> str:
        """Generates a text response from the model."""
        if self.tokenizer is None or self.model is None:
            raise RuntimeError("Model and tokenizer are not loaded. Call load_model() first.")
        ...
        final_answer = response.split("[/INST]")[1].strip() if "[/INST]" in response else response.strip()
