...
class EmotionCascadeSystem:
    def __init__(self):
        ...
        self.glove_path = BASE_DIR / "gloveEmbed/glove.6B.300d.txt"
    ...
    def load_dataset(self):
        ...
        self.df["is_clear"] = (
            (self.df["positive"] == 1) | (self.df["negative"] == 1)
        ).astype(int)

        self.df["is_unclear"] = (
            (self.df["ambiguous"] == 1) | (self.df["neutral"] == 1)
        ).astype(int)
    ...
    def build_tokenizer(self):
        ...
        X = tf.keras.preprocessing.sequence.pad_sequences(
            self.tokenizer.texts_to_sequences(texts), maxlen=self.MAXLEN
        )
    ...
    def load_glove(self):
        ...
        self.embedding_matrix = np.zeros((self.num_words, self.EMBED_DIM))
    ...
    def train(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_all, self.df["is_clear"].values, test_size=0.1, random_state=42
        )
        ...
        X_clear = tf.keras.preprocessing.sequence.pad_sequences(
            self.tokenizer.texts_to_sequences(df_clear["text"]), maxlen=self.MAXLEN
        )

        X_train, X_test, y_train, y_test = train_test_split(
            X_clear, df_clear["positive"].values, test_size=0.1, random_state=42
        )
        ...
        X_unclear = tf.keras.preprocessing.sequence.pad_sequences(
            self.tokenizer.texts_to_sequences(df_unclear["text"]), maxlen=self.MAXLEN
        )

        X_train, X_test, y_train, y_test = train_test_split(
            X_unclear, df_unclear["ambiguous"].values, test_size=0.1, random_state=42
        )
    ...
    def predict(self, text):
        ...
        seq = tf.keras.preprocessing.sequence.pad_sequences(
            self.tokenizer.texts_to_sequences([clean]), maxlen=self.MAXLEN
        )
    ...
    def load_models(self, base_path="emotion_models"):
        ...
        try:
            ...
            print(gate_path)
            print("*********************************" * 20)
            self.model_gate = load_model(
                gate_path,
                custom_objects={"AttentionLayer": AttentionLayer},
                compile=False,
                safe_mode=False,
            )
            self.model_emotion = load_model(
                emotion_path,
                custom_objects={"AttentionLayer": AttentionLayer},
                compile=False,
                safe_mode=False,
            )
            self.model_noise = load_model(
                noise_path,
                custom_objects={"AttentionLayer": AttentionLayer},
                compile=False,
                safe_mode=False,
            ...
            if self.tokenizer is None:
                raise ValueError(
                    "Tokenizer not loaded properly. Model load failed earlier."
                )
